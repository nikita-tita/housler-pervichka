...
export class YandexFeedParser {
  ...
  private get parentElement(): string | undefined {
    return this.elementStack[this.elementStack.length - 2];
  }

  private get currentElement(): string | undefined {
  ...
  private onCloseTag(tagName: string): void {
    if (this.currentOffer) {
      ...
      this.processElement(tagName, text);
    }

    this.elementStack.pop();
  }

  private processElement(tagName: string, text: string): void {
    ...
    switch (tagName) {
      ...
      // Площади и цена (value внутри контейнера)
      case 'value':
        this.handleValueElement(text);
        break;
      ...
      // Контакты
      case 'phone':
        if (this.parentElement === 'sales-agent') {
  ...
  private handleValueElement(text: string): void {
    if (!this.currentOffer) return;
    ...
    switch (parent) {
      case 'area':
        this.currentOffer.areaTotal = value || 0;
        break;
      case 'living-space':
        this.currentOffer.areaLiving = value || null;
        break;
      case 'kitchen-space':
        this.currentOffer.areaKitchen = value || null;
        break;
      case 'price':
        this.currentOffer.price = value || 0;
        break;
    }
  }
