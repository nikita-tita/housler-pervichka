...
## Философия: Compounding Engineering
> "Каждая единица работы должна делать следующую работу ЛЕГЧЕ, а не сложнее."

Не накапливай техдолг. Каждое изменение должно:
- Следовать существующим паттернам
- Документировать неочевидное
- Оставлять код чище, чем было
...
## Часть 13: Цикл разработки (Plan → Work → Review)

### 13.1 PLAN — Планирование (для сложных задач)
```
1. Исследовать кодовую базу
   - Найти похожие реализации (grep, glob)
   - Прочитать существующие паттерны
   - Изучить зависимости

2. Создать план в plans/
...
### 13.2 WORK — Выполнение
```
1. Создать ветку: git checkout -b feature/name
2. Разбить на задачи в TodoWrite
3. Для каждой задачи:
   - Прочитать референсный код
   - Реализовать по паттерну
   - Запустить тесты
   - Отметить выполненным
4. Коммит + PR
```

### 13.3 REVIEW — Проверка (самопроверка)
```
Перед PR пройди чеклист:

БЕЗОПАСНОСТЬ:
□ Нет хардкода секретов
□ Входные данные валидируются
□ SQL параметризован
□ XSS защита на выводе
...
## Часть 14: Справочник проверок

### 14.1 Security Sentinel (безопасность)
```
Проверяй:
- Input validation на всех точках входа
- SQL injection: используй параметризованные запросы
- XSS: экранируй пользовательский ввод
- Hardcoded secrets: grep -r "password\|secret\|key\|token"
- HTTPS для sensitive данных
- CSRF токены в формах
```

### 14.2 Performance Oracle (производительность)
```
Проверяй:
- Time complexity: не хуже O(n log n) без причины
- N+1 queries: используй includes/joins
- Memory leaks: очищай ресурсы
- Bundle size: lazy loading для тяжёлого
- Caching: мемоизация дорогих операций
```

### 14.3 Pattern Recognition (паттерны)
```
Перед написанием нового кода:
1. grep похожую функциональность
2. Прочитай как сделано в проекте
3. Скопируй паттерн
...
║                                                           ║
║   COMPOUNDING: Оставь код лучше чем взял                 ║
║                                                           ║
╚═══════════════════════════════════════════════════════════╝
```
