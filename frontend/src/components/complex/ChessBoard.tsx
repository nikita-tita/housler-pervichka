...
export function ChessBoard({ offers, floorsTotal }: ChessBoardProps) {
  ...
  const { grid, sections, floors } = useMemo(() => {
    ...
    // Create floor range
    const maxFloor = floorsTotal || Math.max(...offers.map(o => o.floors_total || o.floor), 25);
  ...
  const filteredGrid = useMemo(() => {
    if (filterRooms === null) return grid;
              ...
              <div key={section} className="flex-shrink-0 mr-4">
                ...
                {floors.map(floor => {
                  ...
                  return (
                    ...
                    >
                      {cellOffers.length === 0 ? (
                        <div className="w-10 h-full bg-gray-100 rounded opacity-30" />
                      ) : (
