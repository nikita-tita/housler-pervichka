...
export function LocationSection({ complex }: LocationSectionProps) {
  ...
  const infrastructure: InfraItem[] = [
    ...(complex.metro_station ? [{
      ...
      distance: complex.metro_distance ? `${complex.metro_distance} мин` : '',
    }] : []),
    { type: 'school', name: 'Школа №123', distance: '5 мин' },
    { type: 'kindergarten', name: 'Детский сад "Солнышко"', distance: '3 мин' },
    { type: 'hospital', name: 'Поликлиника №45', distance: '10 мин' },
    { type: 'shop', name: 'Пятёрочка', distance: '2 мин' },
    { type: 'park', name: 'Парк Победы', distance: '7 мин' },
  ];
      ...
      <div className="grid lg:grid-cols-[1fr_350px] gap-8">
        ...
        <div>
          ...
          <div className="space-y-4">
            {Object.entries(
              infrastructure.reduce((acc, item) => {
                if (!acc[item.type]) acc[item.type] = [];
                acc[item.type].push(item);
                return acc;
              }, {} as Record<InfraItem['type'], InfraItem[]>)
            ).map(([type, items]) => (
              <div key={type}>
                <div className="flex items-center gap-2 mb-2">
                  {INFRA_CONFIG[type as InfraItem['type']].icon}
                  <span className="text-sm font-medium">
                    {INFRA_CONFIG[type as InfraItem['type']].label}
                  </span>
                </div>
