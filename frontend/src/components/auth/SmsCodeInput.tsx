...
export const SmsCodeInput = forwardRef<HTMLInputElement, SmsCodeInputProps>(
  ({ value, onChange, length = 6, error, disabled }, ref) => {
    ...
    const handleChange = (index: number, newValue: string) => {
      // Обрабатываем вставку нескольких цифр
      ...
      if (pastedDigits.length > 1) {
        ...
        // Фокус на следующую пустую ячейку или последнюю
    ...
    const handleKeyDown = (index: number, e: React.KeyboardEvent<HTMLInputElement>) => {
      if (e.key === 'Backspace') {
        ...
        if (digits[index]) {
          // Если в текущей ячейке есть цифра, удаляем её
          newDigits[index] = '';
        } else if (index > 0) {
          // Если пусто, переходим на предыдущую и удаляем там
          newDigits[index - 1] = '';
          focusInput(index - 1);
        }
    ...
    const handlePaste = (e: React.ClipboardEvent) => {
      ...
      // Фокус на следующую пустую или последнюю
