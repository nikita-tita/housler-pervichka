# ТЗ для мануального тестирования: Housler Новостройки
...
Тестовые аккаунты
  ...
  | Email                        | Роль         | Код входа |
  |------------------------------|--------------|-----------|
  | client@test.housler.ru       | client       | 111111    |
  | client2@test.housler.ru      | client       | 222222    |
  | agent@test.housler.ru        | agent        | 333333    |
  | agent2@test.housler.ru       | agent        | 444444    |
  | admin@test.housler.ru        | admin        | 555555    |
  | agency_admin@test.housler.ru | agency_admin | 666666    |

  Особенности:
  - Все аккаунты с доменом @test.housler.ru
  - Коды постоянные — можно использовать многократно
  - При входе не нужно запрашивать новый код — сразу вводите
  постоянный

  Как войти:
  1. Открыть https://agent.housler.ru/login
  2. Ввести email (например agent@test.housler.ru)
  3. Нажать "Получить код"
  4. Сразу ввести постоянный код (333333 для агента)
  5. Нажать "Войти"
