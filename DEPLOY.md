...
Все проекты настроены на автоматический запуск после ребута сервера:

1. **Docker service** — `systemctl enable docker` (автозапуск при загрузке)
2. **Все контейнеры** — `restart: unless-stopped` (автоматический перезапуск)
